/**
 * Merge OSM + Yelp vets, deduplicate by name+city+state.
 * Prefers Yelp data (has ratings, phone, reviews).
 * Output: src/data/vets.json (unified)
 */
const fs = require("fs");
const path = require("path");

const OSM_PATH = path.join(__dirname, "..", "src", "data", "vets.json");
const YELP_PATH = path.join(__dirname, "..", "src", "data", "yelp-vets.json");
...
for (const r of osm) {
  ...
  const slug = `${slugify(stateName)}-${slugify(r.name || "vet")}-${String(r.id).replace(/[^a-z0-9]/gi,"")}`;

  merged.set(key, {
    id: r.id || `osm-${r.latitude}-${r.longitude}`,
    slug,
    name: r.name || "Veterinary Clinic",
    latitude: r.latitude,
...
for (const r of yelp) {
  ...
  const city = r.city || "";
  const key = normKey(r.name, city, abbr);
  const slug = `${slugify(stateName)}-${slugify(r.name || "vet")}-${slugify(r.id || "")}`;
