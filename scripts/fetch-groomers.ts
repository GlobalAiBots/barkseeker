/**
 * fetch-groomers.ts — Pull US dog groomers from OSM, state-by-state bbox.
 * Tags: shop=pet_grooming, shop=dog_grooming
 * Output: src/data/groomers.json (flat array with state/stateAbbr)
 */
...
async function queryOverpass(bbox: string, attempt = 1): Promise<RawElement[]> {
  try {
    const query = `[out:json][timeout:180];(` +
      ...
      `node["shop"="dog_grooming"](${bbox});` +
      `way["shop"="dog_grooming"](${bbox});` +
      `);out center tags;`;
...
async function main() {
  console.log("=== BarkSeeker: US Groomer Fetch (state-by-state bbox) ===\n");

  const outPath = path.join(__dirname, "..", "src", "data", "groomers.json");
  const seen = new Map<string, Groomer>();
  let total = 0;
  ...
  for (let i = 0; i < states.length; i++) {
    ...
    for (const raw of results) {
      ...
      const t = raw.tags || {};
      const name = t.name || t["name:en"] || t.brand || "Dog Groomer";
  ...
  for (const g of all) byState[g.state] = (byState[g.state] || 0) + 1;

  console.log(`\n=== Final counts ===`);
  Object.entries(byState).sort((a, b) => b[1] - a[1]).forEach(([s, n]) => console.log(`  ${s}: ${n}`));
  console.log(`\nTotal groomers: ${all.length}`);
