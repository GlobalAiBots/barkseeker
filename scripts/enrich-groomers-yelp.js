// Enrich groomers.json with Yelp ratings.
// Only touches records where rating is null or 0. Keeps existing data on no-match.
//
// Run: node --env-file=.env.local scripts/enrich-groomers-yelp.js
// Optional: MAX_RECORDS=500 node --env-file=.env.local scripts/enrich-groomers-yelp.js

import path from "node:path";
import { dice, milesBetween, searchYelp, sleep, readJson, writeJson } from "./_yelp-common.js";

const DATA_PATH = path.resolve("src/data/groomers.json");
const CATEGORY = "petgroomers";
const SLEEP_MS = 200;
const SAVE_EVERY = 50;
const NAME_THRESHOLD = 0.8;
const MAX_MILES = 15; // skip matches whose Yelp location is >15mi from our record
...
async function main() {
  ...
  console.log(`Total groomers: ${data.length}`);
