/**
 * fetch-yelp-vets.js — Query Yelp Fusion for vet in top US cities.
 * Writes src/data/yelp-vets.json (deduped against OSM vets by name+city).
 */
const fs = require("fs");
...
const CATEGORY = "vet";
const OUT_PATH = path.join(__dirname, "..", "src", "data", "yelp-vets.json");
const OSM_PATH = path.join(__dirname, "..", "src", "data", "vets.json");
...
async function main() {
  ...
  console.log(`Loaded ${existingOsm.length} OSM vets for dedup\n`);
  ...
  console.log(`Yelp vets saved: ${byId.size}`);
