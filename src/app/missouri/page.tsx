...
export default function MissouriPage() {
  const stParks = useMemo(() => unified.filter((r) => r.state === "MO"), []);
  ...
  return (
    <div>
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
        ...
        mainEntity: [
          { "@type": "Question", name: "How many dog parks are in Missouri?", acceptedAnswer: { "@type": "Answer", text: `There are ${stParks.length} dog parks in Missouri. BarkSeeker has mapped every off-leash area and fenced park across the state.` } },
          { "@type": "Question", name: "Are dog parks in Missouri free?", acceptedAnswer: { "@type": "Answer", text: "Most dog parks in Missouri are free and open to the public. Some may require a city or county registration tag for your dog." } },
          { "@type": "Question", name: "Are there fenced dog parks in Missouri?", acceptedAnswer: { "@type": "Answer", text: "Yes, many dog parks in Missouri have fully fenced areas. Browse BarkSeeker listings to find fenced off-leash parks near you." } },
          { "@type": "Question", name: "Do dog parks in Missouri have small dog areas?", acceptedAnswer: { "@type": "Answer", text: "Many larger dog parks in Missouri have separate areas for small dogs. Check individual park listings on BarkSeeker for amenity details." } },
          { "@type": "Question", name: "How do I find dog parks near me in Missouri?", acceptedAnswer: { "@type": "Answer", text: `Use BarkSeeker to browse all ${stParks.length} dog parks in Missouri by city. Each listing includes a map and directions.` } },
        ],
      }) }} />
      <section className="py-16 md:py-24 text-center px-4 bg-cream" style={{ backgroundImage: "radial-gradient(circle at 20% 80%, rgba(45,106,79,0.06) 0%, transparent 50%)" }}>
        <p className="text-forest text-sm font-bold tracking-wider uppercase mb-3 font-[Cabin]">Missouri Dog Park Directory</p>
      ...
      <section className="max-w-4xl mx-auto px-4 pt-10 pb-2">
        <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm mb-6">
          <h2 className="font-[Cabin] text-xl font-bold text-charcoal mb-3">Dog Parks in Missouri</h2>
          <p className="text-gray-600 leading-relaxed text-sm">Missouri has {stParks.length.toLocaleString()} dog parks listed on BarkSeeker, with off-leash areas, fenced parks, and dog-friendly trails across the state. Popular areas include St. Louis, Kansas City, and Springfield. Whether your pup needs room to run or a safe space to socialize, you&apos;ll find a park nearby.</p>
        </div>
        <div className="bg-green-50 border border-green-200 rounded-xl p-5 mb-6">
          <h3 className="font-[Cabin] font-bold text-forest mb-3">Tips for Visiting Dog Parks in Missouri</h3>
        ...
        <section className="max-w-6xl mx-auto px-4 pt-8 pb-8">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2">
            {cityMap.slice(0, 16).map(([city, count]) => (
              <Link key={city} href={`/cities/missouri-${city.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "")}`} className="text-left bg-white border border-gray-200 rounded-lg p-3 hover:border-forest hover:shadow-sm transition">
      ...
      <div id="park-list">{selectedCity && <div className="max-w-6xl mx-auto px-4 pb-4"><button onClick={() => setSelectedCity(null)} className="text-sm text-forest hover:underline">&larr; Show all {stParks.length} parks</button></div>}</div><ParkList parks={filteredParks} stateName="Missouri" />

      <section className="max-w-4xl mx-auto px-4 py-10">
        <h2 className="font-[Cabin] text-2xl font-bold text-charcoal mb-4">Missouri Dog Park FAQ</h2>
        <div className="space-y-2">
          {[
            { q: "How many dog parks are in Missouri?", a: `BarkSeeker lists ${stParks.length}+ dog parks across Missouri, including off-leash areas, fenced parks, and dog-friendly trails.` },
            { q: "Are dog parks in Missouri free?", a: "Most dog parks in Missouri are free and open to the public. Some may require a city registration tag for your dog." },
            { q: "Do I need to keep my dog on a leash at Missouri dog parks?", a: "In designated off-leash dog parks, your dog can play freely within the fenced area. Outside of designated off-leash zones, Missouri generally requires dogs to be leashed." },
            { q: "What should I bring to a dog park in Missouri?", a: "Bring water, waste bags, a leash (even at off-leash parks), and treats. Leave food and retractable leashes at home." },
