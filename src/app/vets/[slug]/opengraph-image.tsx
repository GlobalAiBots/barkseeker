...
import { allVets, getVetBySlug, stateNames, slugToAbbr } from "@/data/all-vets";
...
export default async function Image({ params }: { params: { slug: string } }) {
  ...
  if (isState) {
    ...
    title = `Veterinarians in ${name}`;
    subtitle = `${count.toLocaleString()} vets with ratings & reviews`;
  } else {
    const v = getVetBySlug(slug);
    title = v?.name || "Veterinarian";
    subtitle = v ? `${v.city}, ${stateNames[v.stateAbbr] || v.state}` : "";
  }
