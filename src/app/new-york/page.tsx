...
export default function NewYorkPage() {
  const stParks = useMemo(() => unified.filter((r) => r.state === "NY"), []);
  ...
  return (
    <div>
        ...
        <section className="max-w-6xl mx-auto px-4 pt-8 pb-8">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2">
            {cityMap.slice(0, 16).map(([city, count]) => (
              <Link key={city} href={`/cities/new-york-${city.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "")}`} className="text-left bg-white border border-gray-200 rounded-lg p-3 hover:border-forest hover:shadow-sm transition">
      ...
      <div id="park-list">{selectedCity && <div className="max-w-6xl mx-auto px-4 pb-4"><button onClick={() => setSelectedCity(null)} className="text-sm text-forest hover:underline">&larr; Show all {stParks.length} parks</button></div>}</div><ParkList parks={filteredParks} stateName="New York" />
