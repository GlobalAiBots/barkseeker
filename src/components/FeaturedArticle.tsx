...
export default function FeaturedArticle({ listingSlug }: { listingSlug: string }) {
  ...
  return (
    <section className="my-8">
      <h2 className="font-[Cabin] text-xl font-bold text-charcoal mb-4">Dog Owner Tips &amp; Guides</h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {picks.map((p) => (
          <Link key={p.slug} href={`/blog/${p.slug}`} className="group bg-white border border-gray-200 rounded-xl p-5 hover:shadow-md hover:-translate-y-0.5 transition-all">
            ...
            <h3 className="font-[Cabin] font-bold text-charcoal group-hover:text-forest transition mt-2 text-sm leading-snug">{p.title}</h3>
            <p className="text-gray-500 text-xs mt-2 leading-relaxed">{(p.description || "").substring(0, 120)}{(p.description || "").length > 120 ? "..." : ""}</p>
            <span className="text-forest text-xs font-semibold mt-2 inline-block">Read More &rarr;</span>
          </Link>
        ))}
      </div>
    </section>
