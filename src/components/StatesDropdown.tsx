...
export default function StatesDropdown() {
  ...
  return (
    <div className="flex items-center gap-4">
      <Link href="/groomers" className="text-gray-500 hover:text-forest transition text-sm font-medium">Groomers</Link>
      <Link href="/vets" className="text-gray-500 hover:text-forest transition text-sm font-medium">Vets</Link>
      <div ref={ref} className="relative">
        ...
        {open && (
          <div className="absolute top-full left-0 mt-2 bg-white border border-gray-200 rounded-lg shadow-lg z-50 py-1 w-36 max-h-80 overflow-y-auto">
            {states.map((s) => (
              <Link key={s.href} href={s.href} onClick={() => setOpen(false)} className="block px-4 py-2 text-sm text-gray-600 hover:bg-forest/5 hover:text-forest transition">
                {s.name}
              </Link>
            ))}
          </div>
